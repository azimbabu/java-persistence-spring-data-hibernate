...
import com.azimbabu.javapersistence.repository.UserAddressRepository;
...
public class UserAddressManyToManyTest {
  ...
  @BeforeEach
  void setUp() {
    addressRepository.saveAll(generateAddresses());
  ...
  private List<User> generateUsers() {
    ...
    julius.addAddress(addresses.get(2));
    julius.addAddress(addresses.get(1));
    ...
    darren.addAddress(addresses.get(0));
    darren.addAddress(addresses.get(1));
    ...
    marion.addAddress(addresses.get(0));
    marion.addAddress(addresses.get(2));
    ...
    stephanie.addAddress(addresses.get(1));
    stephanie.addAddress(addresses.get(0));

    User burk = new User("burk", LocalDate.of(2020, Month.NOVEMBER, 28));
    burk.setEmail("burk@somedomain.com");
    burk.setLevel(1);
    burk.setActive(true);
    burk.addAddress(addresses.get(1));
    burk.addAddress(addresses.get(2));
  ...
  private List<Address> generateAddresses() {
    addresses.add(generateAddress("11"));
    addresses.add(generateAddress("22"));
    addresses.add(generateAddress("33"));
    return addresses;
  }
  ...
  void manyToManyTest() {
    assertAll(() -> assertEquals(10, userRepository.count()),
        ...
        () -> assertEquals(20, userAddressRepository.count()),
        () -> assertEquals(2, userAddressRepository.countByUserId(users.get(0).getId())));
  }
}
